...
Use recall tools when the answer depends on historical evidence that may have been compacted or lives in another TROVE session.
...
- Broader `session_scope='all'|'session'` is explicit, bounded, raw-message-only archive recovery inside `trove.db`.
...
- `query` follows the same narrow FTS construction rules as `trove_grep`.
- The expansion path is model-backed and bounded by answer/context token limits.

Recommended current-session escalation:

1. `trove_grep` to locate relevant material.
2. `trove_describe` when a known handle needs inspection.
3. `trove_expand_query` when exact detail was compressed away.
...
- `store_id` recovers one raw message and works across TROVE sessions;
...
cited or passed to `trove_evidence_pack`/`trove_compile_evidence`/`trove_compute`. The default remains off
...
Use for semantic discovery across all conversations stored in the local TROVE database. It fuses raw full-text, summary-vector, and verbatim-chunk arms when available and degrades honestly when embeddings are unavailable.
...
- Follow each result's `expand_hint`: current/verbatim hits normally use `trove_expand`; cross-session summaries normally use `trove_load_session`.
...
- Recover a truncated individual row with `trove_expand(store_id=..., content_offset=...)`.
- Set `include_exact_ref=true` when transcript rows will feed exact evidence or computation.

Use Hermes `session_search` for host-tracked sessions that are not present in `trove.db`.
...
For exact raw-message windows, use `trove_grep` with explicit `time_from`/`time_to` instead.
...
- `trove_query_state` queries the same-database assertion sidecar when that feature is enabled.
- `trove_retrieve` is the default-off bounded adaptive controller. It is not required for ordinary recall and must not replace the stable workflow above without measured benefit.

## Operator tools

`trove_status`, `trove_inspect`, and `trove_doctor` report health and metadata. They do not replace content retrieval.
