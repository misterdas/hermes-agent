"""Tests for /trove preset inspection and dry-run application."""

from hermes_trove.command import handle_trove_command
from hermes_trove.config import TROVEConfig
from hermes_trove.engine import TROVEEngine
from hermes_trove.presets import get_preset, preset_match_confidence, preset_status_payload


_PRESET_ENV_VARS = (
    "TROVE_CONTEXT_THRESHOLD",
    "TROVE_FRESH_TAIL_COUNT",
    "TROVE_LEAF_CHUNK_TOKENS",
    "TROVE_CONDENSATION_FANIN",
    "TROVE_INCREMENTAL_MAX_DEPTH",
)
...
def _engine(tmp_path, *, context_length: int = 272_000) -> TROVEEngine:
    config = TROVEConfig(database_path=str(tmp_path / "trove_preset.db"))
...
def test_trove_preset_help_lists_inspection_commands(tmp_path):
    engine = _engine(tmp_path)

    result = handle_trove_command("help", engine)

    assert "- /trove preset show" in result
    assert "- /trove preset suggest" in result
    assert "- /trove preset apply <name> --dry-run" in result


def test_trove_preset_show_exposes_codex_provenance_without_mutating_config(tmp_path):
    engine = _engine(tmp_path)
    before = (engine._config.context_threshold, engine._config.fresh_tail_count, engine._config.leaf_chunk_tokens)

    result = handle_trove_command("preset show codex_gpt_long_context", engine)
    ...
    assert "operator_config_precedence: explicit preset-managed TROVE_* overrides win" in result
...
def test_trove_preset_show_exposes_spark_provenance_without_mutating_config(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
    engine = _engine(tmp_path, context_length=128_000)
    before = (engine._config.context_threshold, engine._config.fresh_tail_count, engine._config.leaf_chunk_tokens)

    result = handle_trove_command("preset show codex_spark_context", engine)
    ...
    assert "TROVE_CONTEXT_THRESHOLD=0.75" in result
    assert "TROVE_FRESH_TAIL_COUNT=16" in result
    assert "TROVE_LEAF_CHUNK_TOKENS=8000" in result
    assert "runtime_mutation: no" in result
    assert before == (engine._config.context_threshold, engine._config.fresh_tail_count, engine._config.leaf_chunk_tokens)


def test_trove_preset_suggest_reports_explicit_operator_config_precedence(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
    monkeypatch.setenv("TROVE_FRESH_TAIL_COUNT", "99")
    engine = _engine(tmp_path)
    engine._config.fresh_tail_count = 99

    result = handle_trove_command("preset suggest", engine)
    ...
    assert "explicit_overrides: TROVE_FRESH_TAIL_COUNT" in result
    assert "TROVE_FRESH_TAIL_COUNT: keep explicit value 99 (preset 24)" in result
    assert "note: suggestion only; no live config was changed" in result


def test_trove_preset_suggest_does_not_lower_codex_gpt55_autoraised_threshold(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
    ...
    assert "TROVE_CONTEXT_THRESHOLD: keep runtime auto-raised value 0.85 (preset 0.75)" in result
    assert "TROVE_FRESH_TAIL_COUNT=24" in result
    assert "TROVE_LEAF_CHUNK_TOKENS=8000" in result
...
def test_trove_preset_suggest_reports_spark_for_128k_context_window(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
    engine = _engine(tmp_path, context_length=128_000)
...
def test_trove_preset_suggest_reports_benchmark_backed_route_confidence_when_host_metadata_matches(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
    engine = _engine(tmp_path, context_length=128_000)
    engine.update_model(
...
def test_trove_preset_suggest_keeps_non_spark_gpt5_128k_context_only(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
    engine = _engine(tmp_path, context_length=128_000)
    engine.update_model(
        model="gpt-5",
...
def test_trove_preset_confidence_does_not_treat_spark_route_as_long_context_match(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
...
def test_trove_preset_suggest_declines_unbenchmarked_context_window(tmp_path):
...
def test_trove_preset_apply_requires_dry_run_and_does_not_mutate_config(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
    engine = _engine(tmp_path)
    before = (engine._config.context_threshold, engine._config.fresh_tail_count, engine._config.leaf_chunk_tokens)

    denied = handle_trove_command("preset apply codex_gpt_long_context", engine)
    dry_run = handle_trove_command("preset apply codex_gpt_long_context --dry-run", engine)

    assert "TROVE preset apply" in denied
    assert "status: denied" in denied
    assert "error: preset apply is preview-only for now; pass --dry-run" in denied
    assert "TROVE preset apply" in dry_run
    assert "status: dry-run" in dry_run
    assert "preset: codex_gpt_long_context" in dry_run
    assert "would_set:" in dry_run
    assert "TROVE_CONTEXT_THRESHOLD=0.75" in dry_run
    assert "TROVE_FRESH_TAIL_COUNT=24" in dry_run
    assert "TROVE_LEAF_CHUNK_TOKENS=8000" in dry_run
...
def test_trove_preset_apply_spark_dry_run_uses_benchmarked_128k_values(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
    engine = _engine(tmp_path, context_length=128_000)
    before = (engine._config.context_threshold, engine._config.fresh_tail_count, engine._config.leaf_chunk_tokens)

    result = handle_trove_command("preset apply codex_spark_context --dry-run", engine)

    assert "TROVE preset apply" in result
...
def test_trove_preset_apply_dry_run_keeps_explicit_managed_env_values(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
    monkeypatch.setenv("TROVE_LEAF_CHUNK_TOKENS", "12000")
    engine = _engine(tmp_path)
    engine._config.leaf_chunk_tokens = 12_000

    result = handle_trove_command("preset apply codex_gpt_long_context --dry-run", engine)

    assert "status: dry-run" in result
    assert "TROVE_LEAF_CHUNK_TOKENS: keep explicit value 12000 (preset 8000)" in result
    assert "TROVE_FRESH_TAIL_COUNT=24" in result
    assert engine._config.leaf_chunk_tokens == 12_000


def test_trove_preset_dry_run_does_not_honor_invalid_env_overrides(tmp_path, monkeypatch):
    _clear_preset_env(monkeypatch)
    monkeypatch.setenv("TROVE_FRESH_TAIL_COUNT", "abc")
    engine = _engine(tmp_path)

    suggest = handle_trove_command("preset suggest", engine)
    apply = handle_trove_command("preset apply codex_gpt_long_context --dry-run", engine)

    assert "explicit_overrides: (none)" in suggest
    assert "invalid_overrides: TROVE_FRESH_TAIL_COUNT=abc" in suggest
    for result in (suggest, apply):
        assert "TROVE_FRESH_TAIL_COUNT=24 (invalid current value abc ignored by runtime; runtime value 32)" in result
