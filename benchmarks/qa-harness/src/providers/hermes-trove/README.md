# hermes-trove provider

Scores [hermes-trove](https://github.com/) — a Python/SQLite lossless
...
hermes-trove is Python-native, so the provider (`index.ts`) drives a long-lived
Python bridge (`bridge/hermes_trove_bridge.py`) over newline-delimited JSON on
...
| `ingest` | Accumulates ONE harness session at a time into a per-container TROVE store on disk: appends the messages to `MessageStore` (session ids/order preserved), builds the **same** deterministic per-session summary the in-house harness uses (`benchmarking.longmemeval.deterministic_session_summary`, imported — never reimplemented), records the summary embedding + conversational-chunk embeddings (batched). |
| `awaitIndexing` | No-op — ingest is fully synchronous. |
| `search` | Calls the **production** `tools.trove_recall` over the container store through a `SimpleNamespace` engine with a fresh, dataset-disjoint `current_session_id`, then maps each hit → `{content, metadata}` and returns the top-k the harness asks for. |
| `clear` | Deletes the container's db + date sidecar. |

The hermes-trove plugin repo is **never modified**: it is made importable via the
...
peeking. `tools.trove_recall`'s snippet (300 chars/hit, ≤25 hits) is the honest
...
hermes-trove's embedding path needs `fastembed` (optional dep). Create a dedicated
...
| Var | Default | Purpose |
|---|---|---|
| `HERMES_TROVE_REPO` | `/Volumes/LEXAR/hermes-work/hermes-trove` | Path to the hermes-trove checkout. |
| `HERMES_TROVE_PYTHON` | `$HERMES_TROVE_REPO/.venv-fastembed/bin/python` | Python interpreter that has `fastembed`. |
| `HERMES_MB_WORKDIR` | `$TMPDIR/hermes-trove-mb` | Base dir for per-container TROVE dbs. |
...
| `TROVE_LONGMEMEVAL_FASTEMBED_CACHE` | fastembed default | FastEmbed model cache dir (point at a roomy volume). |
...
hermes-trove needs **no provider API key** — retrieval is fully local.

## Run

```bash
export HERMES_TROVE_REPO=/Volumes/LEXAR/hermes-work/hermes-trove
...
export TROVE_LONGMEMEVAL_FASTEMBED_CACHE=/Volumes/LEXAR/hermes-work/fastembed-cache
export OPENAI_API_KEY=sk-...            # answerer + judge

# 5-question smoke
bun run src/index.ts run -p hermes-trove -b longmemeval -j gpt-4o -m gpt-4o -l 5 -r smoke-hermes-trove-5q --force

# full LongMemEval_S (500q)
bun run src/index.ts run -p hermes-trove -b longmemeval -j gpt-4o -m gpt-4o -r hermes-trove-fastembed-500q
```
...
Bake these into any numbers reported from a hermes-trove run:
...
- **Retrieval = production `tools.trove_recall` single-shot snippets** (≤25 hits,
  300 chars each). The adapter does not agentically re-expand hits, so this scores
  hermes-trove's one-shot recall payload — not a multi-hop recall→expand agent loop.
