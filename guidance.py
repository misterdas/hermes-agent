"""Product-owned model guidance for safe Hermes-TROVE recall."""
...
def get_recall_policy() -> str:
    ...
    if not policy:
        raise RuntimeError(f"Hermes-TROVE recall policy is empty: {RECALL_POLICY_PATH}")
    if size > MAX_RECALL_POLICY_BYTES:
        raise RuntimeError(
            f"Hermes-TROVE recall policy exceeds {MAX_RECALL_POLICY_BYTES} bytes: {size}"
        )
    return policy
