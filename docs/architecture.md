...
- **Agent tools** - `trove_grep`, `trove_describe`, `trove_expand`, and `trove_expand_query`
...
- **Sensitive-pattern controls** - optional named redaction of API keys, bearer tokens, passwords, and private keys before TROVE stores or summarizes them
- **Storage-boundary payload guard** - media-ish `data:*;base64` and long base64-looking strings are externalized before TROVE writes them to SQLite
- **Diagnostics** - `trove_status`, `trove_inspect`, `trove_doctor`, and optional `/trove` slash commands
...
- current-session retrieval through TROVE tools, not an auxiliary cross-session search step
...
6. **Retrieve** - use TROVE tools to drill into compacted history or synthesize from expanded context
...
Engine
  engine.py              TROVEEngine orchestrator (composes the mixins below)
  ...
  engine_registry.py     process-wide active-clone registry (resolve_active_trove_engine)
...
Ingest, content and retrieval
  ...
  model_routing.py       TROVE auxiliary model-override routing
...
Cohesive groups of stateful `TROVEEngine` methods are lifted verbatim into
`*Mixin` classes in their own files (`compaction.py`, `reconcile.py`,
`aux_session.py`, `placeholder_ledger.py`, …) and mixed back into `TROVEEngine`.
Because the methods still run bound to the engine instance (`self` is the
`TROVEEngine`), they read and write the same runtime state and call the same
...
**Mixin ordering (MRO).** TROVE decomposition mixins are listed *before*
`ContextEngine` in the bases — `class TROVEEngine(SomeMixin, …, ContextEngine)`.
...
`hermes_trove.engine` (for example `summarize_with_escalation` or the `count_*`
...
kept on `TROVEEngine` and reached from the mixin via `self`.
...
been extracted; larger clusters that remain on `TROVEEngine` — session lifecycle
(`on_session_start` / `on_session_end` / rollover), context assembly, the ingest
pipeline, and the TROVE-bypass host-fallback path — are candidates for later,
